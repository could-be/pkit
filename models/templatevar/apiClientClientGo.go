...
{{with .Interface}}
    ...
    // 使用 grpc 直接连接
    func New(grpcAddr string, instr instrumenting.Instrumentation, options ...ClientOption) ({{$Project}}.{{.InterfaceName}}, error) {
        ...
        endpoints := &{{$Project}}.Endpoints{
            ...
            {{.ApiName}}Endpoint: opentracing.TraceClient(instr.Tracer(), "{{.ApiName}}")(
                grpctransport.NewClient(
                    ...
                    encodeGRPCRequest,
                    decodeGRPCResponse,
                    {{$Project}}.{{StartExprToNotExpr .Results.TypeName}}{},
    ...
    func NewWithDiscovery(discoveryConfig *iregister.EtcdConfig, instr instrumenting.Instrumentation) ({{$Project}}.{{.InterfaceName}}, error) {
        ...
        instancer, err := etcdv3.NewInstancer(cli, evolve.ServiceKey({{$Project}}.ServiceName), instr.Logger())
        dlog.Fatal(err)
        ...
        return &{{$Project}}.Endpoints{
            ...
            {{.ApiName}}Endpoint: func(ctx context.Context, request interface{}) (response interface{}, err error) {
                endpointer := sd.NewEndpointer(
                    instancer,
                    endpointFactory({{$Project}}.ServiceName, "{{.ApiName}}", &{{$Project}}.{{StartExprToNotExpr .Results.TypeName}}{}, instr),
                    instr.Logger())
